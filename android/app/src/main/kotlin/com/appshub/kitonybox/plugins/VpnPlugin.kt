...
import com.appshub.kitonybox.RunState
import com.appshub.kitonybox.core.Core
import com.appshub.kitonybox.extensions.awaitResult
import com.appshub.kitonybox.extensions.asSocketAddressText
import com.appshub.kitonybox.extensions.resolveDns
import com.appshub.kitonybox.models.StartForegroundParams
import com.appshub.kitonybox.models.VpnOptions
import com.appshub.kitonybox.modules.SuspendModule
import com.appshub.kitonybox.services.BaseServiceInterface
import com.appshub.kitonybox.services.KitonyBoxService
import com.appshub.kitonybox.services.KitonyBoxTileService
import com.appshub.kitonybox.services.KitonyBoxVpnService
...
data object VpnPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    @Volatile
    private var kitonyBoxService: BaseServiceInterface? = null
    @Volatile
    ...
    private val connectivity by lazy {
        KitonyBoxApplication.getAppContext().getSystemService<ConnectivityManager>()
    }
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(className: ComponentName, service: IBinder) {
            ...
            isBind = true
            isBinding.set(false)
            kitonyBoxService = when (service) {
                is KitonyBoxVpnService.LocalBinder -> service.getService()
                is KitonyBoxService.LocalBinder -> service.getService()
    ...
    override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        ...
        if (GlobalState.currentRunState == RunState.START && kitonyBoxService == null) {
    ...
    private suspend fun startForeground() {
        ...
        try {
            kitonyBoxService?.startForeground()
        } catch (e: Exception) {
    ...
    fun updateNotificationIcon() {
        scope.launch {
            runCatching {
                ...
                (kitonyBoxService as? KitonyBoxService)?.resetNotificationBuilder()
                (kitonyBoxService as? KitonyBoxVpnService)?.resetNotificationBuilder()
                kitonyBoxService?.startForeground()
            }.onFailure {
    ...
    fun handleUpdateNotificationSpeed(profileName: String, speedInfo: String) {
        if (profileName != GlobalState.currentProfileName ||
            ...
            TileService.requestListeningState(
                context,
                ComponentName(context, KitonyBoxTileService::class.java)
            )
        }
    ...
    fun updateNotificationSpeed(profileName: String, speedInfo: String) {
        scope.launch {
            runCatching {
                (kitonyBoxService as? KitonyBoxVpnService)?.updateNotificationSpeed(profileName, speedInfo)
            }.onFailure {
    ...
    fun getStatus(): Boolean {
        return GlobalState.runLock.withLock {
            GlobalState.currentRunState == RunState.START && kitonyBoxService != null
        }
    }
        ...
        scope.launch {
            try {
                val prepareIntent = try {
                    android.net.VpnService.prepare(KitonyBoxApplication.getAppContext())
                } catch (e: Exception) {
                    null
                }
    ...
    private suspend fun performStartCore(
        ...
        var fd: Int? = 0
        try {
            fd = kitonyBoxService?.start(currentOptions)
        } catch (e: Exception) {
        ...
        val canStart = GlobalState.runLock.withLock {
            if (GlobalState.currentRunState != RunState.START) {
                kitonyBoxService?.stop()
                false
            } else true
        }
        if (!canStart) return

        com.appshub.kitonybox.core.Core.startTun(
            fd = fd ?: 0,
        ...
        GlobalState.runLock.withLock {
            ...
            if (currentOptions.dozeSuspend) {
                ...
                suspendModule = SuspendModule(KitonyBoxApplication.getAppContext())
    ...
    private fun protect(fd: Int): Boolean {
        ...
        while (retries < 5) {
            val success = runCatching {
                (kitonyBoxService as? KitonyBoxVpnService)?.protect(fd) == true
            }.getOrDefault(false)
    ...
    private fun resolverProcess(
        ...
        uidPageNameMap.getOrPut(nextUid) {
            KitonyBoxApplication.getAppContext().packageManager?.getPackagesForUid(nextUid)
                ?.firstOrNull() ?: ""
        }
    }.getOrElse {
    ...
    fun handleStop(force: Boolean = false) {
        ...
        GlobalState.runLock.withLock {
            ...
            GlobalState.updateRunState(RunState.STOP)
            ServicePlugin.notifyRunStateChanged(RunState.STOP)
            serviceRef = kitonyBoxService
            wasBound = isBind
            shouldForceStop = force || kitonyBoxService == null
        }
        ...
        runCatching {
            if (wasBound) {
                KitonyBoxApplication.getAppContext().unbindService(connection)
                isBind = false
            }
            kitonyBoxService = null
        }.onFailure {
        ...
        val context = KitonyBoxApplication.getAppContext()
        if (shouldForceStop) {
            context.stopService(Intent(context, KitonyBoxVpnService::class.java))
            context.stopService(Intent(context, KitonyBoxService::class.java))
        }
    ...
    private fun bindService() {
        ...
        try {
            ...
            val intent = Intent(
                ...
                if (options?.enable == true) KitonyBoxVpnService::class.java else KitonyBoxService::class.java
            )
            val res = KitonyBoxApplication.getAppContext().bindService(intent, connection, Context.BIND_AUTO_CREATE)
            if (!res) {
                isBinding.set(false)
                bindTimeoutJob?.cancel()
                bindTimeoutJob = null
