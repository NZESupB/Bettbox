...
suspend fun Service.createKitonyBoxNotificationBuilder(): NotificationCompat.Builder =
    withContext(Dispatchers.IO) {
        val defaultComponent = ComponentName(packageName, "com.appshub.kitonybox.MainActivity")
        val lightComponent = ComponentName(packageName, "com.appshub.kitonybox.MainActivityLight")
        val darkComponent = ComponentName(packageName, "com.appshub.kitonybox.MainActivityDark")
        ...
        val pendingIntent = withContext(Dispatchers.Main) {
            PendingIntent.getActivity(this@createKitonyBoxNotificationBuilder, 0, intent, flags)
        }
        ...
        NotificationCompat.Builder(this@createKitonyBoxNotificationBuilder, GlobalState.NOTIFICATION_CHANNEL).apply {
            setSmallIcon(R.drawable.ic)
            if (largeIconBitmap != null) {
                setLargeIcon(largeIconBitmap)
            }
            setContentTitle("KitonyBox")
...
fun Service.ensureNotificationChannel() {
    ...
    if (channel == null || channel.importance != NotificationManager.IMPORTANCE_LOW) {
        manager?.createNotificationChannel(
            NotificationChannel(GlobalState.NOTIFICATION_CHANNEL, "KitonyBox Service", NotificationManager.IMPORTANCE_LOW)
        )
    }
}
