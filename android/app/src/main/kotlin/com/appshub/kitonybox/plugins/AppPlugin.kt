package com.appshub.kitonybox.plugins
...
import com.appshub.kitonybox.KitonyBoxApplication
import com.appshub.kitonybox.GlobalState
import com.appshub.kitonybox.R
import com.appshub.kitonybox.extensions.awaitResult
import com.appshub.kitonybox.extensions.getActionIntent
import com.appshub.kitonybox.models.Package
...
class AppPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware {
    ...
    private val iconCacheDir by lazy {
        File(KitonyBoxApplication.getAppContext().cacheDir, "app_icons").apply {
            if (!exists()) mkdirs()
        }
    }
    ...
    private fun initShortcuts(labels: Map<*, *>) {
        val toggleShortcut = ShortcutInfoCompat.Builder(KitonyBoxApplication.getAppContext(), "toggle")
            .setShortLabel(labels["toggle"]?.toString() ?: "Toggle")
            .setIcon(createBitmapIcon(KitonyBoxApplication.getAppContext(), R.drawable.ic_shortcut_toggle))
            .setIntent(KitonyBoxApplication.getAppContext().getActionIntent("CHANGE"))
            .build()

        val startShortcut = ShortcutInfoCompat.Builder(KitonyBoxApplication.getAppContext(), "start")
            .setShortLabel(labels["start"]?.toString() ?: "Start")
            .setIcon(createBitmapIcon(KitonyBoxApplication.getAppContext(), R.drawable.ic_shortcut_start))
            .setIntent(KitonyBoxApplication.getAppContext().getActionIntent("START"))
            .build()

        val stopShortcut = ShortcutInfoCompat.Builder(KitonyBoxApplication.getAppContext(), "stop")
            .setShortLabel(labels["stop"]?.toString() ?: "Stop")
            .setIcon(createBitmapIcon(KitonyBoxApplication.getAppContext(), R.drawable.ic_shortcut_stop))
            .setIntent(KitonyBoxApplication.getAppContext().getActionIntent("STOP"))
            .build()

        ShortcutManagerCompat.setDynamicShortcuts(
            KitonyBoxApplication.getAppContext(), 
            listOf(startShortcut, stopShortcut, toggleShortcut)
        )
    }

    private fun isSystemInDarkMode(): Boolean {
        val nightModeFlags = KitonyBoxApplication.getAppContext().resources.configuration.uiMode and
            Configuration.UI_MODE_NIGHT_MASK
        return nightModeFlags == Configuration.UI_MODE_NIGHT_YES
    }

    private fun isAndroidTV(): Boolean {
        val uiMode = KitonyBoxApplication.getAppContext().resources.configuration.uiMode
    ...
    private fun tip(message: String?) {
        if (GlobalState.flutterEngine == null) {
            Toast.makeText(KitonyBoxApplication.getAppContext(), message, Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun openFcmDiagnostics(): Boolean {
        val context = KitonyBoxApplication.getAppContext()
    ...
    private fun updateExcludeFromRecents(value: Boolean?) {
        val am = KitonyBoxApplication.getAppContext().getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager
    ...
    private fun getIconSizePx(): Int {
        val density = KitonyBoxApplication.getAppContext().resources.displayMetrics.density
    ...
    private fun getDefaultIconBytes(): ByteArray? = runCatching {
        drawableToPngBytes(KitonyBoxApplication.getAppContext().packageManager.defaultActivityIcon, getIconSizePx())
    }.getOrNull()
    ...
    private suspend fun getPackageIconBytes(packageName: String, forceRefresh: Boolean = false): ByteArray? =
        withContext(Dispatchers.IO) {
            val pm = KitonyBoxApplication.getAppContext().packageManager ?: return@withContext null
            runCatching {
    ...
    private suspend fun getPackages(forceRefresh: Boolean = false): List<Package> = withContext(Dispatchers.IO) {
        ...
        val pm = KitonyBoxApplication.getAppContext().packageManager ?: return@withContext emptyList()
        val selfPackageName = KitonyBoxApplication.getAppContext().packageName
    ...
    fun requestVpnPermission(callBack: () -> Unit) {
        ...
        val intent = runCatching { VpnService.prepare(KitonyBoxApplication.getAppContext()) }.getOrNull()
        if (intent != null) {
    ...
    fun requestNotificationsPermission() {
        ...
        if (ContextCompat.checkSelfPermission(KitonyBoxApplication.getAppContext(), Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED) return
        activityRef?.get()?.let {
    ...
    private fun isIgnoringBatteryOptimizations(): Boolean =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val powerManager = KitonyBoxApplication.getAppContext().getSystemService(android.content.Context.POWER_SERVICE) as? android.os.PowerManager
            powerManager?.isIgnoringBatteryOptimizations(KitonyBoxApplication.getAppContext().packageName) ?: false
        } else true

    private fun requestIgnoreBatteryOptimizations() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return
        val intent = Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS).apply {
            data = Uri.parse("package:${KitonyBoxApplication.getAppContext().packageName}")
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
        runCatching { KitonyBoxApplication.getAppContext().startActivity(intent) }
            .onFailure {
                runCatching {
                    KitonyBoxApplication.getAppContext().startActivity(Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS).apply {
                        addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                    })
                }
            }
    ...
    private fun setLauncherIcon(useDarkIcon: Boolean) {
        ...
        val defaultComponent = android.content.ComponentName(packageName, "com.appshub.kitonybox.MainActivity")
        val lightComponent = android.content.ComponentName(packageName, "com.appshub.kitonybox.MainActivityLight")
        val darkComponent = android.content.ComponentName(packageName, "com.appshub.kitonybox.MainActivityDark")
        ...
        com.appshub.kitonybox.services.NotificationComponentCache.invalidate()
    ...
    private fun hasCameraPermission(): Boolean =
        ContextCompat.checkSelfPermission(KitonyBoxApplication.getAppContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED
    ...
    private fun openAppSettings() {
        ...
        activityRef?.get()?.startActivity(intent) ?: run {
            ...
            KitonyBoxApplication.getAppContext().startActivity(intent)
        }
    }
