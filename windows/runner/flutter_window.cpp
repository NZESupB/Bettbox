...
bool HasLegacyIconSettings() {
  HKEY hKey;
  if (RegOpenKeyExW(HKEY_CURRENT_USER, KITONYBOX_REG_KEY, 0, KEY_READ,
                    &hKey) != ERROR_SUCCESS) {
    return false;
  }
...
void RemoveLegacyIconRegistryKeys() {
  HKEY hKey;
  if (RegOpenKeyExW(HKEY_CURRENT_USER, KITONYBOX_REG_KEY, 0, KEY_SET_VALUE,
                    &hKey) == ERROR_SUCCESS) {
...
constexpr const wchar_t kFlutterWindowProp[] = L"KitonyBoxFlutterWindow";
